...
| `Hr_holds_exactly_three_permissions_and_none_touches_money` [Verified: 2026-08-22 @ `CatalogueCompletenessTests.cs` -> `Hr_holds_exactly_three_permissions_and_none_touches_money`] | **Changed after all, by a different ruling.** This row predicted "no change" and was right about `ProjectCreate` — but `UserRead` (D-055 §2) added HR's third permission, so the test was renamed from `..._two_permissions_and_neither_...`. It also gained the money assertion its old name had only claimed. See D-056 §2. |
