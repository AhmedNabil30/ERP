...
public sealed class UserScreenTests
{
    ...
    // ⚠️ `AssertPortalAccountExistsAsync` lived here, private to this file, until 2026-09-08. It is now
    // `E2ESession.AssertPortalAccountExistsAsync` — unchanged, and moved rather than copied. It was
    // private, so `AuditScreenTests`, written later against the same account, went without it and
    // passed on an empty database for four days (`V-35-K` §15.3). An absence control that only one
    // suite can reach is an absence control the next suite will not have.
