...
**Slice:** 2 (Masters) · **Epic:** Masters · **Points:** 3 (`stories/backlog.md`'s slice-2 table) · **Status:** **NOT-BUILT.** Refined 2026-09-09 by the BA. **Two Definition-of-Ready boxes are unticked, and the story's one real event — a person moving between the populations — is unruled.**
**Spec:** **§10** (*"Two populations, one source each … Nobody appears in both"*), **§2** · **Decisions:** D-016 (🟡), D-044 ruling 4
**Register:** `stories/questions-for-karim.md` → **`Q12`** (open, slice-2-wide), **`Q69`** (blocking)
...
| 4 | ⛔ **What happens when a day labourer goes onto the payroll is `Q69`, and it is this story's centre.** The entity's own note says such a person *"needs a decision from HR about their history, not a silent field edit"* [Verified: 2026-09-09 @ `src/Domain/MasterData/Employee.cs` -> `EmployeeKind`] — **that is the code recording a gap, not a ruling.** §10 says nobody is in both; it does not say what happens to somebody who moves. Three readings are all ordinary: refuse it outright, archive the old record and create a new one, or keep one record and move it. **Each does something different to the person's engagement history, and history is the part that cannot be undone** | **`Q69`** — uncited, therefore asked, not decided |
...
| 6 | `EmployeeManage`, `CompanyWide`, **no assignment**. HR settled by §2 and §10; the Owner grant stands on **`Q12`** [Verified: 2026-09-09 @ `src/Domain/Authorization/PermissionCatalogue.cs` -> `Permission.EmployeeManage`] | §2, §10 · D-044 ruling 4 · **`Q12`** |
...
**AC-208-E — HELD on `Q69`: a person who moves between the populations**
Given a day labourer who goes onto the payroll
When HR tries to record it
Then — **held.** `Q69` decides between refusing, archiving-and-recreating, and moving one record. **Nothing is implemented until it is ruled**, and `AC-208-C` holds the line meanwhile: today the answer is *refuse*, which is the reversible one — no history is destroyed by it
...
| DoR item | |
|---|---|
| Every criterion is Given / When / Then | ✅ — `AC-208-A` … `AC-208-G`. **`AC-208-E` is written and held** |
| Stable `AC-208-<LETTER>` ids, appended never inserted | ✅ |
| Every business rule cites a `spec.md` section or a D-number | ✅ — rules 1–8; rule 4 cites `Q69` and is marked as a question rather than sourced |
| No uncited rule | ✅ |
...
| The audit record it writes is stated | ✅ — rule 7: a refusal writes none, and any permitted move must write one |
| **QA has written at least one scenario that fails if the rule is broken** | ⛔ **Not met.** `qa/slice-2/` does not exist and no `TC-` range is allocated [Verified: 2026-09-09 — `qa/` holds `slice-1`, `questions.md`, `README.md`, `risk-register.md` and `strategy.md`]. Five criteria are marked *(fails if the rule is broken)*, and **`AC-208-C` is red against the code as it stands** — there is no edit endpoint to refuse, and the error it names is returned by nothing. **QA's to write** |
| Story-currency citations dated with a stable identifier | ✅ |
| Not `BLOCKED` on an open question | ⛔ **Not met — `Q69`** at the story's centre, **`Q12`** slice-wide, and **`Q70`** through `AC-208-B`'s mechanism |

**Flip the trailer to `READY` when `Q69` and `Q12` are ruled and QA's cases land.** `Q70` need not be
ruled first, but `AC-208-B` must be re-read the day it is.
...
- **Deleting `MasterDataErrors.EmployeeKindIsImmutable` or adding a `SetKind`.** `AC-208-C` requires
  one or the other; **which one is `Q69`'s consequence**, not a preference to be exercised now.

## Questions

| # | Question | Owner |
|---|---|---|
| **`Q69`** | **New, raised here and blocking.** What happens when a day labourer goes onto the payroll — refuse, archive and re-register, or move the one record? And what becomes of his engagement history either way | **Karim** |
