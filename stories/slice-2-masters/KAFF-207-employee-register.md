...
| 2 | **New.** Is `Department` required on the staff file, or optional? D-139 §7 names the field and not its optionality; it is built optional today (commit `c5b3a24`) ahead of the ruling. **Does not block** — the story is buildable either way and `AC-207-G` holds under both | **Karim** |
