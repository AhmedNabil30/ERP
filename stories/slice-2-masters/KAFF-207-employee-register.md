...
<!-- kaff id=KAFF-207 slice=2 points=5 state=VERIFIED verdict=CONDITIONAL at=54ff044 on=2026-09-13 -->
...
**Register:** `stories/questions-for-karim.md` → **`Q12`** (✅ answered, D-129 §1), **`Q68`** (✅ **ANSWERED IN FULL** — reference number by D-130 §6; the staff-file field list by D-139 §7 and D-144 §2)
...
| 4 | ✅ **What a salaried staff record carries beyond §10's four worker fields is ruled — D-139 §7, D-144 §2.** The staff file is **Name, Phone, National ID, Department, Job Title, HiredOn** — Nabil named the first five as *"nothing more"*, and D-144 §2 keeps `HiredOn` on top of that list because HR and payroll need it later. `NationalId`, `JobTitle` and `HiredOn` are already on the entity [Verified: 2026-09-09 @ `src/Domain/MasterData/Employee.cs` -> `SetStaffDetails`]; **`Department` is not**, and Backend has already added it as optional (commit `c5b3a24`), ahead of this ruling landing. ⛔ **Whether `Department` must be required is NOT ruled** — D-139 §7 names the field, not its optionality — and it is registered below rather than decided here | **D-139 §7 · D-144 §2** |
...
- **i18n:** `hr.employee.list_title`, `hr.employee.create_title`, `hr.employee.edit_title`,
  ...
  `hr.employee.field.national_id`, `hr.employee.field.department`, `hr.employee.field.job_title`,
  `hr.employee.field.hired_on`, `hr.employee.archive_action`, `enum.employee_kind.salaried`,
  `enum.employee_kind.day_labour`, plus the existing `errors.master.*` keys for the domain guards.
...
**AC-207-C — the employee code is generated, unique, and cannot be typed** *(fails if the rule is broken, restated 2026-09-12 — `V-37-E`: the create request carries no `Code` member, so a submitted code cannot collide)*
Given `CreateEmployee.Request`, which has no `Code` property [Verified: 2026-09-12 @ `src/Domain/MasterData/Employee.cs` -> `Create`]
When two create requests are sent at the same instant
Then each employee is given a distinct, system-generated code — the same generated shape as `KAFF-119`'s client code (D-130 §6) — and the guarantee that the two never collide is the database's unique index on the generated value, not a read-then-write in the handler
And a request body that carries a `code` property regardless is ignored; the response's code is always the one the server generated
...
And the form renders exactly the staff file D-139 §7 and D-144 §2 name — Name, Phone, National ID, **Department**, Job Title, `HiredOn` — and no field beyond that list
...
| Every business rule cites a `spec.md` section or a D-number | ✅ — rules 1–12; rule 5 is re-cited to `D-130 §6`, rule 4 to `D-139 §7` / `D-144 §2` |
| No uncited rule | ✅ |
...
| Not `BLOCKED` on an open question | ✅ — `Q68` is answered in full (reference number: D-130 §6; field list: D-139 §7 / D-144 §2) and `Q12` (D-129 §1) is answered. `Q70` reaches `AC-207-D`'s mechanism through `KAFF-209` and is unrelated to this box. **New, does not block:** whether `Department` is required (see *Questions*) |

**Flip the trailer to `READY` when QA's cases land.** Backend still owes the switch from a typed to a
generated employee code (see above) before `AC-207-G` is true of the running system, and adding
`Department` to the entity and both screens.
...
| # | Question | Owner |
|---|---|---|
| **`Q68`** | ✅ **ANSWERED IN FULL.** The reference number is **generated**, not typed (D-130 §6, `AC-207-G`). **The staff file is Name, Phone, National ID, Department, Job Title, `HiredOn`** (D-139 §7, D-144 §2) | **Closed** |
