...
<!-- kaff id=KAFF-200 slice=2 points=5 state=VERIFIED verdict=CONDITIONAL at=35b0550 on=2026-09-12 -->
...
**Register:** `stories/questions-for-karim.md` → **`Q60`** (schema half ✅ answered, D-129 §2, refined by D-136/D-144; the data half — Kaff's real trades and markups — stays open as **`Q75`**), **`Q61`** (✅ answered, D-129 §3 / D-130 §1), and **`Q12`** (✅ answered, D-129 §1)
...
| 3 | An imported item carries exactly: `code · descriptionAr · descriptionEn · unit · bab (by Code) · costPrice · baseSellRate`. **No `status` column.** The template has two description columns, Arabic and English, not one — the UI shows whichever matches the signed-in user's language. Nothing else is read from the sheet, and a column the sheet carries that this list does not name is **not** imported into a field invented for it | §4.1 · **D-144 §§3–4** |
...
| 6 | An item's باب must already exist and is looked up **by its `Code`, not by its name** — the template's باب column carries a code. The item's `BabId` is required and there is no unassigned باب [Verified: 2026-09-08 @ `src/Domain/MasterData/CatalogueItem.cs` -> `BabId`]. **The template carries exactly rule 3's item fields and no column that creates a باب** (D-129 §2), so the أبواب are created first, through `KAFF-204`, and an import row only references one that already exists | §2 · §4.1 · **D-129 §2 · D-144 §4** |
| 6a | **Every imported item is `Active`.** The template carries no `status` column at all — D-018's 🟡 open question about what values `status` may hold is answered for this story by removing the column, not by enumerating it: an import writes no status other than `Active` | **D-144 §5** |
...
Then N catalogue items exist, each carrying the code, Arabic description, English description, unit, باب (resolved by Code) and the cost price and base sell rate of its row, and each is `Active`
...
And a rate with more than four decimals in the sheet is **stored at four decimals through `Money`**, the same as any other caller — **the import adds no refusal rule of its own** (**D-144 §6**'s reading of D-008)
...
Then it is accepted only if its columns match the template's — exactly: code, description (Arabic), description (English), unit, باب (by Code), cost price, base sell rate — **no `status` column** — and a file with an extra or a missing column is refused, naming what the template requires and what the file carried instead
...
| **QA has written at least one scenario that fails if the rule is broken** | ✅ **Met 2026-09-09** — `qa/slice-2/test-cases.md`, `TC-2-001`…`TC-2-098`. ⚠️ **`AC-200-B` needs a re-cased test**, not a new ruling — its two-behaviours ambiguity is now closed by **D-144 §6**: a rate beyond four decimals is stored at four through `Money`, no refusal. QA re-cases `TC-2-098` (or whichever case names it) against the single behaviour |
...
**Flip the trailer to `READY` when QA's `AC-200-B` case is re-cased against D-144 §6.** Everything else
this story was waiting on is ruled: `Q60`'s schema half (D-129 §2, D-136, D-144 §§3–5), `Q61` (D-129 §3
/ D-130 §1), `Q12` (D-129 §1), and D-008's rounding policy for this story (D-144 §6).
...
| # | Question | Owner |
|---|---|---|
| **`Q60`** | ✅ **Schema half ANSWERED — D-129 §2, and D-136's three follow-on template questions (one description column or two; باب by name or code; what `status` does) are all closed by D-144 §§3–5.** `AC-200-I` is written against the closed shape. **The data half — Kaff's real ~40 trades and their markups — is still open, re-registered as `Q75`**, and this story seeds none of it | **Karim** — `Q75` only |
...
| 2 | ✅ **CLOSED for imported rows — D-144 §5.** Every imported item is `Active` and the template carries no `status` column. `CatalogueItemStatus`'s full range (whether anything beyond `Active`/`Archived` exists) stays a separate, narrower question for whoever next needs a third value — not raised by this story | **Closed for this story** · D-018, D-144 §5 |
