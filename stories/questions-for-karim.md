...
> ### ⚠️ Four new rows, 2026-09-08 — and **three of them block the slice that is open**
>
> **`Q60`, `Q61`, `Q62`** and **`Q63`** were raised by the BA refining slice 2 (`STATUS.md` sprint-6
> item 6). Three of them block a slice-2 story at its centre; `Q63` blocks a QA case.
>
> ⛔ **`STATUS.md` reads *"Slice 2 needs none of them"*, and that sentence is about the seven rows in
> its own table — it is not true of this register.** `stories/backlog.md`'s slice-2 section has said
> **"Blocked by: Q12 and Q13"** since the slice was estimated, *"both due **before** this slice opens
> rather than during it"*, and **`Q12` is still open.** Every catalogue and باب story now names it in
> its permissions rule, because `Q12` decides whether the Owner keeps `CatalogueManage` and
> `BabManage`. It removes or keeps a grant; it reshapes no criterion. **The three new rows are the
> ones that reshape stories.**
>
> **Not one of the four was answered by the session that raised them**, and `Q61` is the one to watch:
> its answer is written on the board already, in a story *title*, and a title is not a ruling.
...
| **Q64** | ⛔ **STILL OPEN — D-129 §6.** *"Same for 62 63 64"* has content for `Q62` and `Q63`; **`Q64` is a data-exposure boundary, not a pragmatism question**, and is not closed by it. **Nothing is blocked**: D-055 §2 already rules **name and role, nothing else**, and HR's screen is built to that. This row asks only whether Karim, *having now seen the screen*, wants that list changed. Original question: **"Your own user list shows you everyone in Kaff — their name, their username, their role, their department, their phone, and whether their account is switched on. HR has to put people onto projects, so HR needs to see a list of people too. We were told HR may see **names and roles and nothing else**. Is that still what you want — or now that you have seen the screen, do you want HR to see anything more, or anything less?"** Say why it is being asked: **HR's list does not exist.** The permission was created for it on 2026-08-22 and no screen and no endpoint was ever built [Verified: 2026-09-08 — no `RequirePermission` names `Permission.UserRead` anywhere under `src/Api/Features/`], so HR still cannot name a single person to staff a project, which is the whole reason the role exists. **This does not reopen D-055 §2** — it invokes that ruling's own clause, *"Revisit if HR needs to see anything about a user beyond name and role — which is a question for Karim"*, and asks him to confirm the field list **before** somebody builds the surface rather than after | **`S-010`'s user picker, and HR's half of `KAFF-115`** (DEFERRED to slice 1b, 8 points). **Blocks nothing that is built.** ⛔ **And it is not answered by opening `GET /api/users` to HR** — `AC-127-K` holds that line, and `Q42`'s verbatim warning is *"Do not close it by handing HR the Owner's user list"* | **Drafted 2026-09-06** in `proposals/V-34-A-user-list-acceptance-criteria.md` §5, **as `Q58`, and never merged** — that session could not edit this file and said so. **`Q58` has since been taken** by the تشوينات recovery schedule (2026-09-07), so the question is registered here on **2026-09-08** under a free number. Raised by Verifier finding **`V-34-F`** and by `ux/slice-1-flows.md`'s standing note under S-009: *"HR cannot assign somebody it cannot name."* **A question drafted in a proposal is not an open question until it is in this file** — that is what this row is the evidence for |
| **Q70** | **"What are your أبواب — the trades — and what markup percentage does each one carry?"** Say why it is being asked: **`Q60`'s template defines the columns; it cannot fill them.** `spec.md` §4.2's *concrete 15%* and *finishes 30%* are **examples, not defaults to seed** (`KAFF-204` rule 7), and an agent building the tree against an empty answer will reach for those two numbers and they will look like data. Ask it alongside the template: the sheet Karim fills in **is** the answer | **`KAFF-204` — blocking its data, not its behaviour.** The tree, the create/edit screen and the markup rules can all be built and tested against fixtures; what cannot be invented is which trades exist and what each is worth | New, **2026-09-09**. This is **`Q60`'s second half**, split out when D-129 §2 closed the first. `Q60` asked two things and was marked answered for one |
...
| **Q59** | **"When you open the list of the people who use the system — and the list of your clients — what order do you want the names in? Alphabetical, the newest first, or something else?"** Say why it is being asked: **nothing in `spec.md` or in the screen designs states an order for either list.** And *"alphabetical"* is not yet an instruction in Arabic — ا / أ / إ can be sorted together or apart, and the two readings put different names in different places. | **Nothing.** Both lists work. What it blocks is a **test case**: QA cannot assert an order nobody has ruled, so `AC-127-A` and `AC-126-A` are covered for direction, labels, geometry and overflow and **not** for order — and a sort that is wrong for Kaff would go unnoticed indefinitely | New, 2026-09-07, raised by QA writing the first cases for the four frontend stories (`V-34-J`). `KAFF-127`'s own **F-127-3** already named it — the user list orders by full name, *"sensible, unruled, and harmless … if the order matters to Kaff, it is a question for Karim and nobody has asked it"* — **and this row is that ask.** The client list has no ruled order either |
| **Q63** | ✅ ***ANSWERED* — D-129 §5, 2026-09-09, by Nabil.** Ordered **by باب, then by item code within each باب**. The Arabic-collation caveat (ا / أ / إ) is **not** answered and is the Architect's, not Karim's. Original question: **"When you open the price list, what order do you want the items in — by their code, grouped by باب, alphabetically by the description, or something else?"** Say why it is being asked: **nothing in `spec.md` or in the screen designs states an order for this list either**, and the Arabic-collation caveat on `Q59` — ا / أ / إ together or apart — applies here word for word. **Ask it in the same breath as `Q59`.** It is the same question about a third list, and asking them together is what makes the pattern visible rather than producing three separate rulings that disagree | **Nothing.** Same shape as `Q59`: the search works in any order, so what it blocks is a **QA case** — `AC-203-I` is written and held, and an order that is wrong for Kaff would otherwise go unnoticed indefinitely | New, 2026-09-08, raised by the BA refining **`KAFF-203`**. Held as `AC-203-I` rather than answered |
| **Q60** | ✅ ***ANSWERED in part* — D-129 §2, 2026-09-09, by Nabil.** A **standardized downloadable template** is provided to the Technical Office, so the shape is defined rather than discovered. ⛔ **The data half is NOT answered and moved to `Q70`** — a template defines columns, it cannot say what Kaff's ~40 trades are or what each one's markup is. Original question: **"Can we see the price-list spreadsheet you keep now?"** And with it: what the columns are called, whether it is one sheet or several, whether the باب is written as a name or a code, and **whether that same file also carries the أبواب and their markups, or whether the trades are set up first and the items loaded into them.** Say why it is being asked: `spec.md` §4.1 names the fields an item carries — `code · description · unit · bab · costPrice · baseSellRate · status` — and says the catalogue is *"loaded from Excel at setup"*. **It says nothing about the file.** A column mapping guessed from the field list is a mapping that fails on Kaff's real sheet, and the failure is silent when a column happens to line up | **`KAFF-200` — blocking.** `AC-200-A` says *"in the agreed shape"* and `AC-200-I` is held open, because a criterion cannot assert a mapping nobody has given. Also decides **`KAFF-204`'s data**: the real ~40 trades and their markups. `spec.md` §4.2's *"concrete 15%, finishes 30%"* are examples, not Kaff's rates | New, 2026-09-08, raised by the BA refining **`KAFF-200`** |
| **Q61** | ✅ ***ANSWERED* — D-129 §3, 2026-09-09, by Nabil.** The import **loads the good rows and returns a row-level validation report** naming every row it could not take. ⚠️ **This reverses `KAFF-200`'s "all-or-nothing" title and rule 7**, which are the BA's to rewrite. One reading was chosen of two — see D-129 §3. Original question: **"If one row in that spreadsheet is wrong — a price missing, a trade that does not exist, the same code twice — should the system refuse the whole file and change nothing, or load all the good rows and give you a list of the ones it could not?"** Say why it is being asked: **`stories/backlog.md`'s own story title says *"all-or-nothing"*, and no `spec.md` section and no D-number says it.** Both behaviours are defensible and both are ordinary; they differ in what happens on the day a 600-row file has one bad row in it, which is the day this will matter | **`KAFF-200` — blocking.** It is the story's rule 7, written as the board's title and marked uncited rather than sourced, and `AC-200-H` is held open on it | New, 2026-09-08, raised by the BA refining **`KAFF-200`**. **The title was not treated as a ruling** — `process/agile.md`'s Definition of Ready is explicit: *"An uncited rule is a question for Karim, not a story"* |
| **Q62** | ✅ ***ANSWERED* — D-129 §4, 2026-09-09, by Nabil.** A second import is **accepted, its effect shown, and confirmed by a human** — which is `KAFF-201`'s own "deliberate, reviewed act" and honours §4.1's "not an ongoing sync". Original question: **"You load the price list from a spreadsheet once, at the start. If somebody imports a second spreadsheet later — because prices changed, or the first file was wrong — what should happen? Refuse it, and make them correct the items one at a time on the screen? Add only the items that are new, and leave the ones already there exactly as they are? Or update the prices of the existing ones as well?"** Say why it is being asked: §4.1 tells us what it must **not** be — *"Excel import is not an ongoing sync"* — and names no replacement. **A prohibition is not an instruction** | **`KAFF-201` — blocking, and at the centre of it rather than at an edge.** Two of the three readings make it a 2-point change and one makes it a one-line gate. Five criteria that hold under all three are written; `AC-201-F`, the one that cannot, is held. §4.4 protects signed BOQs under every reading, so **nothing unsafe is waiting on the answer** — only the behaviour | New, 2026-09-08, raised by the BA refining **`KAFF-201`**. The board's title — *"a deliberate, reviewed act"* — describes a **gate in front of** whichever reading is chosen, and was **not** read as a fourth answer |
| **Q12** | ✅ ***ANSWERED* — D-129 §1, 2026-09-09, by Nabil rather than Karim.** *"The owner have all the privileges on all the system."* The Owner **keeps** `CatalogueManage`, `BabManage`, `EmployeeManage`, `SubcontractorManage` and `OpportunityManage`. Original question: *"When you said you can create and edit all master data — did you mean all of it, or the three you listed: clients, suppliers and bank accounts?"* ⚠️ **This does not permit self-approval** — "nobody creates and approves the same movement" checks the actor of a movement, not the permissions held. | slice 2 — **now unblocked** | **D-129 §1.** Was **D-045 #2**, open since D-044 ruling 4. QA **F-15**'s five matrix cells all resolve to "may" |
...
| **Q14** | **"At extract 1 the client pays an extra 75,000 for material delivered to site, and it comes off later extracts as that material is installed — correct?"** One sentence, and it confirms the D-034 fix. **⚠️ Narrowed 2026-09-07, by the BA cutting `KAFF-300`: this row is sometimes described as asking *which ledger*, and it does not.** The ledger is decided — D-034 ruled تشوينات a **liability**, *"money received for work not yet done — structurally identical to `ClientAdvance`"*, and `AccountType.MaterialAdvance` is in the catalogue [Verified: 2026-09-07 @ `src/Domain/Treasury/AccountType.cs` -> `AccountType.MaterialAdvance`]. **Q14 confirms a mechanic that was inferred from §15's own arithmetic.** Still open, still owed, and still not answerable by any agent. **The schedule — how much comes off each extract — is `Q58`, above, and Q14 does not cover it** | slice 3 · **the §15 gate**. **It does not block `KAFF-300`:** D-034 requires the fixture *"failing or skipped, but present"* before slice 3 opens, and `spec.md` is the business truth until Karim says otherwise. It gates **`AC-300-I`'s figures** — if the mechanic is contradicted, extract 1's 240,000 and two totals move | Kickoff Q8, still open |
| **Q58** | **"When the client pays you 75,000 up front for material sitting on site, and it comes back off later extracts as the material gets built in — how do you decide how much comes off each time? Is it the value of that material actually installed in the period, or a share fixed when the payment was made, or something else?"** Say why it is being asked: **`spec.md` §15 gives the amounts and no rule that produces them.** The worked example recovers **45,000** at extract 2 and **30,000** at extract 3. That is not 25% of period work value — that is the *advance* recovery, §16 assumption 2, and it is a separate column in the same table — and it is not proportional to work value either: 45,000 : 30,000 runs the opposite way to 300,000 : 400,000. §6.4 and D-034 both describe تشوينات as *"recovered as the material is installed"*, which names the event and not the arithmetic. **Q14 does not cover this** — Q14 confirms the mechanic; this asks for the schedule | **Nothing today. `KAFF-300` transcribes §15's three figures as given data and writes no formula** (`AC-300-I`). **The deadline is slice 5's calculator**, which cannot compute a column from a rule nobody has stated | New, 2026-09-07. Raised by the BA cutting **`KAFF-300`**, walking §15's table row by row against the rules that produce each column: hold (20%, §15 header), advance recovery (25% of period work value, §16 assumption 2) and work value are all sourced; **the تشوينات column is the only one with no rule behind it anywhere in `spec.md`.** Registered rather than inferred — an invented recovery schedule reconciles §15 perfectly, because §15's own figures are what it would be fitted to, and would then be wrong on the first real project |
...
| **N11** | **Should `audit_records` be partitioned by month from the start, rather than converted at slice 9?** Nabil ruled Q54 with **PostgreSQL table partitioning by month**, to drop whole historical partitions once the retention period expires — the only mechanism that expires the IP address without violating append-only. **The ruling places it at slice 9. The consequence is due now:** converting a *populated* table to a partitioned one is a new table plus a data migration plus a swap, **on a table that is append-only and trigger-protected** — precisely the table you least want to rewrite. **The deadline is not slice 9; it is before the first production rows exist**, and the cheapest moment is before slice 3 starts writing real money history | Nothing today. **Slice 3 is the real deadline**, not slice 9. **⚠️ 2026-09-07: N11 now has a story — `stories/slice-1-foundation/KAFF-129-partition-audit-records-by-month.md`, 8 points, `NOT-BUILT`.** It had none for thirteen days while sitting as item 1 of `STATUS.md`'s *"What is actually next"*, so it carried no points and could not be pulled. **The question itself is not closed by the story:** four sub-questions stay with the Architect — the timezone that defines a month boundary, what creates next month's partition, how `FindMissingGuardsAsync` covers a table set that grows on its own, and whether `postings` ever follows | New, 2026-08-25, raised by **D-072 §3**. Routed to the Architect by Nabil's own instruction — *"route it to the Architect as a question"* — rather than settled |
| **N12** | **The user list's shape — four things nobody has decided, gathered into one row because they are one conversation.** ① **Which state chip `S-006` selects by default.** It draws `[ All ] [ Active ] [ Inactive ]` and does not say which is on; the payload carries every account either way (`AC-127-L`), so this is a screen decision and it belongs to KAFF-127, not to an endpoint criterion. ② **The collation Arabic names sort under.** `ListUsers`' handler orders by `FullName` [Verified: 2026-09-08 @ `src/Api/Features/Users/ListUsers/Handler.cs` -> `HandleAsync`] and the schema states no collation. ⚠️ **This is the Architect's half of `Q59`, not a duplicate of it**: `Q59` asks Karim **what order Kaff wants**; this asks what implements it, and *"alphabetical"* is not yet an instruction in Arabic. ③ **Paging.** There is none — the whole `users` table on every request, with one correlated assignment sub-query per row. Fine at Kaff's headcount and nothing states a bound. ④ **Where the revocation names live.** `ux/slice-1-flows.md` -> `S-008` requires them *"in the same response that describes the user"*; there is **no `GET /api/users/{userId}`**, so they are on every list row. **④ is now ratified rather than open** — `AC-127-N`'s *"plus what `AC-127-M` requires"* licenses the ninth member through a criterion that cannot be satisfied without it; the row is kept so that **building a detail endpoint later is a decision and not a drift**. **None of the four is a business rule and none goes to Karim** | Nothing built. **② is the only one that is cheaper before real data exists than after** | **Drafted 2026-09-06** in `proposals/V-34-A-user-list-acceptance-criteria.md` §5 and **never merged**; that session could not edit this file. Registered **2026-09-08** by the BA reconciling the two `V-34-A` passes. ①③④ come from `S-006`/`S-008` read against the handler; ② from the `OrderBy`. The story reached ②③ independently as its own findings **`F-127-3`** and **`F-127-2`**, and ① and the search/filter half as **`F-127-1`** — **two passes, one row** |
