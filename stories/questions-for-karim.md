...
| **N9** | **Does the staff sign-in endpoint refuse a `Role.Client` credential outright, or accept it and let the client reach nothing?** Q33 ruled the portal is a separate host and settled the *screen*; **nothing has been ruled about the endpoint**. `KAFF-101a` rule 16 (`KAFF-101a:55`) still says a client credential *"authenticates"* there, which predates the ruling. Not cosmetic: accepting it mints a valid session cookie on the staff origin for a user who has no business holding one. It reaches nothing today — but *"reaches nothing"* is a property of the current permission catalogue, and refusing at the door is a property of the door | Nothing in slice 1: there is no portal shell and a client holds no staff permission. It decides one line of KAFF-101a rule 16, and it should be decided before KAFF-101a is built rather than after | UX Q-UX-20, merged by **SM-8**. Related to **N7**, which owns the deployment shape; this owns the door |
