...
### D-146 · Architect — amends D-141 per D-144 §1: salaried phones refuse through a partial unique index, every other employee match stays warn-and-acknowledge; the archived cross-population case is derived, the active one stays with Nabil (`Q80`) · 2026-09-12

D-144 §1 rules the business: a duplicate **salaried** phone is **refused**, and day labour, subcontractors
and suppliers stay warn-and-acknowledge (D-139 §1). It asks the Architect to amend D-141 and to rule the
cross-population case if it can be derived. This entry does both, as far as the rulings go. **D-141's text
is not rewritten.** The changes to it are listed below by section.

**D-141 is not built yet, so it is built once, as amended here.** As of today the employee index is still
table-wide and unique
[Verified: 2026-09-12 @ `src/Infrastructure/Persistence/Configurations/MasterDataConfigurations.cs` -> `ux_employees_phone`],
`PhoneMatches` still lives in the Clients feature
[Verified: 2026-09-12 @ `src/Api/Features/Clients/PhoneMatches.cs` -> `PhoneMatches`], and
`EmployeePhoneTaken` is still declared and returned
[Verified: 2026-09-12 @ `src/Domain/MasterData/MasterDataErrors.cs` -> `EmployeePhoneTaken`],
[Verified: 2026-09-12 @ `src/Api/Features/Employees/CreateEmployee/Handler.cs` -> `IsPhoneCollision`].
There is one migration, not a D-141 migration followed by a D-146 one.

**How `Kind` is stored, confirmed before choosing the filter:** every enum is stored as its member name
by convention [Verified: 2026-09-12 @ `src/Infrastructure/Persistence/KaffDbContext.cs` -> `ApplyEnumsAsStrings`],
and the table's existing check constraint already compares `kind` by name
...
So the filter is `kind = 'Salaried'`. A numeric filter (`kind = 1`) would match nothing.

#### What changes in D-141

| D-141 | Was | Now |
|---|---|---|
| §1, the migration | `ux_employees_phone` becomes non-unique `ix_employees_phone` | `ux_employees_phone` is dropped. **Two** indexes replace it: `ux_employees_salaried_phone`, unique on `phone_normalised`, filtered `kind = 'Salaried'`; and `ix_employees_phone`, non-unique on `phone_normalised`, for the warning lookup. The subcontractor and supplier halves are unchanged. The migration is renamed `Q70PhoneIndexes`, because "warn only" is no longer true of the whole of it |
| §3, existing data | No row conflicts | Still true. The table-wide unique index guarantees there are no duplicates today, so the partial index builds on any existing data |
| §5, the acknowledgement | A flag lets any match through | The flag **never** lets a salaried-to-salaried match through. That refusal is checked first (point 3 below) |
| §6, deletions | `EmployeePhoneTaken`, both `IsPhoneCollision` catches and the `employee_phone_taken` keys are deleted | **Withdrawn.** All of them stay (point 2 below) |
| *Tests*, the grep | `employee_phone_taken` must not appear in `tests/` | **Withdrawn.** Every remaining hit must be a salaried-to-salaried scenario |
| *Tests*, `The_phone_index_is_not_unique` | Asserts that every `ix_*_phone` is non-unique | Replaced by test 1 below |
| *What this does not decide*, "Salaried staff" | Open, for Nabil | **Closed** by D-144 §1 and point 1 below |

#### Decision

1. **The mechanism is a partial unique index plus a non-unique lookup index.** In EF, one column carries
   two indexes only through the **named** overload:
   `HasIndex(e => e.PhoneNormalised, "ux_employees_salaried_phone").IsUnique().HasFilter("kind = 'Salaried'")`
   and `HasIndex(e => e.PhoneNormalised, "ix_employees_phone")`. Calling the unnamed `HasIndex` twice
   returns the **same** index, and the second call silently reconfigures the first. The migration is
   generated by EF, not written by hand.
   - **Archived salaried rows are inside the predicate.** The index covers every salaried row, active or
     archived, because that is the predicate D-141 and D-144 §1 both name, and D-144 §1 does not
     exclude archived records. The consequence is listed under *What this does not decide*.
2. **The error is the existing `MasterDataErrors.EmployeePhoneTaken`** (`master.employee_phone_taken`,
   `errors.master.employee_phone_taken`). It is kept with the same key and the same wording in both
   catalogues. Its meaning narrows from "any employee" to "another salaried record", and the wording
   (*"already belongs to another employee record"*) is still accurate. Its doc comment is rewritten to
   name `ux_employees_salaried_phone` and the salaried scope. A new key would change the SPA, both
   catalogues and two tests to say the same thing.
3. **Which paths return it:**
   - **`CreateEmployee` with `Kind = Salaried`**, when any salaried record, active or archived, has the
     same normalised phone. The check is one query before the save:
     `Employees.AnyAsync(e => e.PhoneNormalised == phone && e.Kind == EmployeeKind.Salaried)`.
     The answer is `409 EmployeePhoneTaken` **whatever `AcknowledgedDuplicatePhone` says**, and nothing
     is written: no row and no audit record. This check runs **before** D-141 §5's acknowledgement rule.
     The query gives the clean answer. **The index is the guarantee**: the existing `IsPhoneCollision`
     catch stays and now matches the constraint name `ux_employees_salaried_phone`, so two requests
     racing each other get the same `409`.
   - **`EditEmployee` on a salaried record**, when the new phone matches **another** salaried record.
     The same query adds `&& e.Id != id`, and the same catch applies. An edit that does not change the
     phone never matches the record itself. `Kind` cannot be edited (KAFF-208 rule 3), so an edit can
     never move a record into the other population.
   - **Nothing on the day-labour side returns it.** That covers HR's create and edit with
     `Kind = DayLabour`, and D-140's Site Engineer registration, which can never create a salaried
     record. Every match there is D-141 §5's warn-and-acknowledge.
   - **`POST /api/employees/phone-check` is unchanged.** It warns with every match's name and does not
     say which kind the match is. On a salaried form, the refusal appears when the operator submits.
     *Not added: a kind on `PhoneMatch`. Add it if UX asks for the refusal before submit.*
4. **The cross-population case, split into the half that can be derived and the half that cannot.**
   - **(a) Derived: the match is an ARCHIVED record of the other kind. The result is
     warn-and-acknowledge, in both directions.** D-130 §7 rules that a day labourer who joins the payroll
     is archived and re-registered as a new salaried record. A person keeps his phone, so refusing this
     match would make D-130 §7's ruled flow impossible, and D-130 §7 rejects refusal in terms
     (*"refusing outright is not a rule, it is an absence of one"*). It warns rather than passing
     silently, because D-139 §1 is the ruling that governs a worker phone match and D-141 §4 puts
     archived rows in the match set. The reverse direction (an archived salaried record, then a new day
     labourer on the same phone) rests on the test D-130 §7 applies: §10's *"nobody appears in both"*
     holds at every point in time, because the archived record is no longer in its population. **This
     releases `AC-208-E`'s hold.** D-141's renamed test
     `Re_registering_the_same_person_by_phone_after_archiving_warns_and_succeeds_once_acknowledged`
     now asserts a ruled behaviour, not a provisional one.
   - **(b) ⛔ Not derived: the match is an ACTIVE record of the other kind** (`AC-208-B`: an active day
     labourer's phone on a salaried create, or an active salaried record's phone on a day-labour
     registration). §10 says nobody appears in both, and D-139 §1 says worker duplicates warn. A shared
     phone is evidence that two records are one person, not proof of it (D-049 ruling 8: two parties can
     share one number). Whether that evidence is strong enough to refuse is a business choice that
     nobody has written down. **`Q80` stays with Nabil, narrowed to active records.** What each answer
     means for the build:
     - **Refuse:** handler code only. No index can express it: a unique index on the phone
       `WHERE is_active` would also refuse two day labourers, which D-139 §1 forbids. The salaried create
       and the day-labour create each check for an active record of the other kind, and return a new
       `409` error named when the ruling is given. The check is race-safe only if the handler first takes
       `pg_advisory_xact_lock` keyed on `phone_normalised` in the same transaction. On the Site
       Engineer's route, the refusal uses D-140 point 6's restricted form and shows no name.
     - **Warn:** nothing changes. Today's behaviour stands and `AC-208-B` is released as
       warn-and-acknowledge.
     - **Meanwhile the handler warns and asks for an acknowledgement** on an active cross-population
       match. D-141 §5 is the last mechanism ruled for every match that D-144 §1 did not carve out, so it
       stays until Nabil answers. **No test pins this as correct**, and `AC-208-B` stays HELD. Each
       acknowledged match writes `DuplicatePhoneAcknowledged` against the matched id. If Nabil says
       refuse, the pairs created meanwhile can be found in the audit trail.
5. **D-140 point 6 is unaffected.** A day labourer registered from site whose phone matches a salaried
   record, active or archived, still gets the restricted match and needs the acknowledgement.

#### Why

- **Why an index and not a query alone:** D-144 §1 says *refused*, and D-049 ruling 8 and D-141 both
  state that a refusal lives in the database. A read-then-write lets two simultaneous saves through. The
  query exists only so the normal case returns a clean `409` without relying on the exception.
- **Why keep `EmployeePhoneTaken`:** D-141 §6 deleted it because it could never run again. Once it can
  run again, that reason is gone.
- **Why `Q80` is not ruled here:** point 4(b) weighs two written rules against each other. Choosing which
  one wins is the business decision that CLAUDE.md says an agent does not make.

#### What Backend builds

1. D-141 as written, with the rows of *What changes in D-141* applied.
2. `EmployeeConfiguration`: the two named indexes in point 1. Generate `Q70PhoneIndexes`.
3. `CreateEmployee` and `EditEmployee`: the salaried check from point 3, placed before the
   acknowledgement rule. Keep both `IsPhoneCollision` catches, pointed at `ux_employees_salaried_phone`.
4. `MasterDataErrors.EmployeePhoneTaken`: rewrite the doc comment only. The key and both catalogue
   entries stay.

#### What Frontend builds

- On a salaried create or edit, a `409 errors.master.employee_phone_taken` shows as a refusal, not as the
  duplicate-warning dialog. The operator cannot acknowledge it.
- D-141 asked Frontend to remove `employee_phone_taken` from `employees.api.ts`'s doc comment. Instead,
  the comment is corrected to say the error applies to salaried records only.

#### Tests (all in `tests/Api.Tests/`)

1. `The_phone_indexes_are_non_unique_except_the_salaried_partial_index` replaces D-141's
   `The_phone_index_is_not_unique`. It reads `pg_indexes` and asserts all of the following:
   - `ux_employees_phone`, `ux_subcontractors_phone` and `ux_suppliers_phone` do not exist;
   - `ix_employees_phone`, `ix_subcontractors_phone` and `ix_suppliers_phone` exist, and their
     `indexdef` does not contain `UNIQUE`;
   - `ux_employees_salaried_phone`'s `indexdef` starts with `CREATE UNIQUE INDEX` and contains `WHERE`
     and `'Salaried'`. PostgreSQL prints the filter back as `WHERE ((kind)::text = 'Salaried'::text)`, so
     the test asserts on those parts, not on the whole string.
2. `A_salaried_phone_cannot_be_registered_twice_and_the_refusal_is_the_partial_unique_index`, renamed
   from `The_same_phone_cannot_be_registered_twice_and_the_refusal_is_the_unique_index`
   [Verified: 2026-09-12 @ `tests/Api.Tests/CreateEmployeeTests.cs` -> `The_same_phone_cannot_be_registered_twice_and_the_refusal_is_the_unique_index`].
   The second create enters the phone in a different form (`0020…` against `010…`) and sends
   `acknowledgedDuplicatePhone: true`. The response is `409 errors.master.employee_phone_taken`, and no
   row and no audit record are written.
3. `A_salaried_phone_matching_an_archived_salaried_record_is_refused`.
4. `Editing_a_salaried_record_onto_another_salaried_phone_is_refused`.
5. `Two_salaried_creates_racing_on_one_phone_leave_one_row`: one `201` and one `409 employee_phone_taken`,
   which proves the catch path.
6. `A_day_labourer_matching_an_archived_salaried_record_warns_and_succeeds_once_acknowledged`, for
   point 4(a) in reverse.
7. D-141's `Re_registering_the_same_person_by_phone_after_archiving_warns_and_succeeds_once_acknowledged`
   stays as D-141 wrote it, for point 4(a).
8. D-141's `An_unacknowledged_duplicate_employee_phone_is_refused_and_writes_nothing` and
   `Employee_phone_match_is_on_the_normalised_form` use **two day-labour records**. With salaried records
   they would hit the refusal instead.
9. `A_day_labourer_cannot_be_registered_again_as_salaried_with_the_same_phone`
   ...
   asserts a refusal on an **active** cross-population match, which is one of `Q80`'s two answers. It
   becomes `[Fact(Skip = "HELD on Q80 — decisions.md D-146 §4(b)")]` and keeps its name and body. When
   Nabil answers, it is either unskipped with the new error key or rewritten as warn-and-acknowledge. It
   is not rewritten now.

#### What this does not decide — questions for Nabil

- **`Q80`, narrowed to active records** (point 4(b)): an active day labourer and an active salaried
  record on one phone. Should the save be refused, or warned about and acknowledged?
- **Rehiring a salaried leaver on the same phone.** Point 1 puts archived salaried rows inside the
  refusal, and `Employee` has no unarchive path
  [Verified: 2026-09-12 @ `src/Domain/MasterData/Employee.cs` -> `Archive`] (a Grep of the file for
  `Unarchive` or `Reactivate` finds nothing). So a salaried person who leaves and is later rehired on the
  same number cannot be registered again. If Nabil wants rehiring to be possible, there are two ways:
  add `AND is_active` to the filter, or add an unarchive path. **Which one is his choice**, and it is
  not made here.
